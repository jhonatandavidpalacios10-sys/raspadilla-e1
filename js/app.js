...
export function renderProductosVenta() {
    ...
    // Filtro por Sede para los vendedores
    ...
    if(filtrados.length === 0) { grid.innerHTML = `<div class="col-span-full flex justify-center py-10 text-slate-500 text-sm">No hay productos disponibles.</div>`; return; }
    ...
    filtrados.forEach(p => {
        ...
        html += `<div data-id="${p.id}" data-categoria="${p.categoria}" class="producto-card bg-slate-800 border border-slate-700 rounded-xl md:rounded-2xl p-2 md:p-3 flex flex-col items-center text-center transition-all relative overflow-hidden ${blockCls}">${badgeLocal}${badgeHtml}<div class="w-10 h-10 md:w-14 md:h-14 bg-gradient-to-br ${cCls} rounded-full flex items-center justify-center mt-3 mb-2 shadow-lg"><i data-lucide="${p.categoria === 'vaso' ? 'cup-soda' : 'package'}" class="w-5 h-5 md:w-7 md:h-7 text-white"></i></div><h3 class="text-[10px] md:text-sm font-bold text-slate-800 dark:text-white mb-1 leading-tight line-clamp-2">${p.nombre}</h3><p class="text-${p.categoria==='vaso'?'sky':'emerald'}-500 font-black text-xs md:text-sm mt-auto">${formatMoney(p.precio)}</p></div>`;
    });
...
function toggleSabor(n) {
    ...
    document.querySelectorAll('.sabor-btn').forEach(btn => {
        ...
        if(saboresElegidos.includes(nm)) { btn.classList.add('bg-sky-500', 'text-white', 'border-sky-500'); btn.classList.remove('bg-slate-100', 'dark:bg-slate-900'); chk.classList.replace('border', 'bg-white/30'); chk.classList.replace('border-slate-400', 'border-transparent'); chk.classList.replace('dark:border-slate-600', 'border-transparent'); chk.innerHTML = '<i data-lucide="check" class="w-3 h-3 text-white"></i>'; }
        else { btn.classList.remove('bg-sky-500', 'text-white', 'border-sky-500'); btn.classList.add('bg-slate-100', 'dark:bg-slate-900'); chk.classList.replace('bg-white/30', 'border'); chk.classList.replace('border-transparent', 'border-slate-400'); chk.innerHTML = ''; }
    });
...
function calcularVuelto() {
    ...
    if(document.querySelector('input[name="metodo_pago"]:checked')?.value === 'efectivo') {
        ...
        document.getElementById('txt-vuelto').textContent = v >= 0 ? formatMoney(v) : 'S/ 0.00'; document.getElementById('txt-vuelto').classList.toggle('text-red-500', v < 0);
    }
}

// ========================================================
// PROCESAR COBRO (Optimizacion a Segundo Plano / Background)
// ========================================================
function procesarCobroFinal() {
    ...
    try {
        ...
        // -------------------------------------------------------------
        // MAGIA: ENVIAR A SEGUNDO PLANO (Sin "await")
        // La UI se limpiará al instante, y Firebase subirá esto en background.
        // -------------------------------------------------------------
        ...
        if(window.mostrarToast) window.mostrarToast('Venta Procesada', `El Ticket #T-${tId.split('-')[1]} ha sido registrado rápido.`, 'emerald');
        
        // Actualizar visualmente el stock local de la UI sin pedirle nada al servidor
        cr.forEach(item => {
        ...
        renderProductosVenta(); // Se actualiza la grilla al toque

    } catch (err) { 
        console.error(err); 
        if(window.mostrarToast) window.mostrarToast('Error', 'Hubo un error local procesando la venta.', 'red');
    }
}
