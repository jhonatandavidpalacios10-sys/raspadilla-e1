    ...
    if (accs.length > 0) {
        ...
        list.innerHTML = accs.map(a => `
            <div class="relative group">
                <button data-action="quick-login" data-email="${a.email}" data-pass="${a.pass}" class="flex flex-col items-center gap-2 p-3 bg-slate-900 border border-slate-700 hover:border-sky-500 rounded-xl transition-all w-20 sm:w-24 active:scale-95">
                    <div class="w-10 h-10 bg-sky-500/20 text-sky-400 rounded-full flex items-center justify-center shrink-0">
                        <i data-lucide="user" class="w-5 h-5"></i>
                    </div>
                    <span class="text-[10px] sm:text-xs font-bold text-white truncate w-full text-center">${a.username}</span>
                </button>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Dibujamos perfiles guardados al iniciar
    renderProfiles();

    // Eventos para alternar vistas del Login
    ...
    document.getElementById('btn-show-profiles')?.addEventListener('click', () => {
        renderProfiles(); // Vuelve a dibujar y mostrar la sección de perfiles
    });

    // Delegación para Perfiles Rápidos
    document.getElementById('saved-profiles-list')?.addEventListener('click', async (e) => {
        ...
        if (btnLogin) {
            ...
            btnLogin.innerHTML = '<div class="w-10 h-10 flex items-center justify-center shrink-0"><i data-lucide="loader-2" class="w-6 h-6 animate-spin text-sky-400"></i></div><span class="text-xs font-bold text-sky-400 mt-2">Cargando...</span>';
            if(window.lucide) window.lucide.createIcons();
            ...
            try {
                ...
                if(window.lucide) window.lucide.createIcons();
                if(window.mostrarAlerta) window.mostrarAlerta('Credenciales Caducadas', 'La contraseña de esta cuenta fue cambiada o el usuario no existe. Inicia sesión manualmente para actualizar el perfil.', 'amber');
                removeAccount(email);
            }
        }
    });
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user && !datosCargados) {
            ...
            datosCargados = false; 
            // Cuando alguien cierra sesión, refrescamos y mostramos la pantalla de perfiles
            renderProfiles();
        }
    });
    ...
    if (lf) {
        lf.addEventListener('submit', async (e) => {
            ...
            try { 
                ...
                // GUARDAR PERFIL PARA INICIO RÁPIDO
