...
export function initVentas() {
    ...
    // FIX: El formulario de Cargo/Descuento no tenía listener de submit.
    // Al pulsar "Aplicar al Ticket" el navegador recargaba la página y se perdía el carrito.
    document.getElementById('form-ajuste')?.addEventListener('submit', confirmarAjuste);

    // FIX: Sincronizar la UI de método de pago con el radio "Efectivo" que viene
    // marcado por defecto (antes el área "Paga con / Vuelto" quedaba oculta hasta
    // cambiar de método y volver).
    const metodoInicial = document.querySelector('input[name="metodo_pago"]:checked')?.value || 'efectivo';
    window.toggleMetodoPago(metodoInicial);
