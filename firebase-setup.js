...
export async function initInventario() {
    ...
    tabs.forEach((tab, index) => {
        tab.addEventListener('click', () => {
            tabs.forEach(t => {
                t.classList.remove('text-sky-400', 'text-amber-400', 'text-amber-500', 'border-sky-400', 'border-amber-400', 'border-amber-500', 'border-b-2');
    ...
    items.forEach(p => {
        ...
        tr.innerHTML = `
            ...
            <td class="p-3 text-center">
                <div class="flex justify-center gap-1.5 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
